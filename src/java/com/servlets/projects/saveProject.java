    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           String checkbox_display= request.getParameter("display");
            ...
            Project project=new Project( client_id,price,order,status,display,starting_date,ending_date,demo_link,deployed_link,part.getSubmittedFileName(),Description,  Tittle);
            try
            {
               int i =ProjectDao.save(project);
            if (i == 1) {
